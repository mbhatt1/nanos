...
- P0: Lexical canonicalization (symlinks not resolved)
- P1: Consider symlink resolution policy

**Residual Risk:** Symlink-based attacks in P0 (mitigated by not resolving symlinks)
...
- Control-plane events are counted toward budget
...
### 9.1 Denial Detection

- Check `last_deny` for recent denials
- Review rate-limited denial log
- Query audit log for patterns

### 9.2 Audit Trail

- All effects logged with trace_id
- Policy hash recorded for each entry
- Chain verification detects tampering

### 9.3 Policy Update

- Load new policy via kernel interface
- Old policy remains until new is validated
- Policy change is audited
