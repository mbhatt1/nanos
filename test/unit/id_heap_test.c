...
static boolean basic_test(heap h)
{
    ...
    for (u64 page_order = 0; page_order <= MAX_PAGE_ORDER; page_order++) {
        ...
        for (u64 alloc_order = 0; alloc_order <= (u64)(LENGTH_ORDER - page_order); alloc_order++) {
            u64 n = U64_FROM_BIT(alloc_order);
            msg_debug(">>> allocations of %ld page(s) ... ", n);

            for (u64 i = 0; i < pages; i += n) {
...
static boolean random_test(heap h, heap rh, u64 page_order, u64 churn)
{
        ...
        for (u64 i = 0; i < VEC_LEN; i++) {
            for (u64 j = 0; j < VEC_LEN; j++) {
                ...
                if ((i_first >= j_first && i_first <= j_last) ||
                    ...
                    msg_err("%s error: results %ld and %ld intersect", func_ss, i, j);
                    goto fail;
                }
            }
        }
    ...
    if (heap_allocated(id) > 0) {
        msg_err("%s: heap allocated (%ld) should be zero; fail", func_ss, heap_allocated(id));
        return false;
    }
    destroy_heap(id);
    return true;
  fail:
    ...
    for (u64 i = 0; i < VEC_LEN; i++) {
        msg_err("%ld\t(%ld,\t%lx)", i, alloc_size_vec[i], alloc_result_vec[i]);
    }
    return false;
}
...
static boolean alloc_subrange_test(heap h)
{
    ...
    u64 pages_remaining = SUBRANGE_TEST_LENGTH / PAGESIZE;
    /* test alloc within unaligned range */
    u64 alloc_size = 1;
    u64 allocs = SUBRANGE_TEST_INIT_ALLOCS - 1; /* subtract skipped page from head align */
    u64 expect = SUBRANGE_TEST_MIN + PAGESIZE;
    u64 start = SUBRANGE_TEST_MIN + 1;
    u64 end = SUBRANGE_TEST_MIN + (allocs + 1) * PAGESIZE + 1;
    u64 res;
    for (u64 i = 0; i < allocs; i++) {
    ...
    if (pages_remaining != allocs + 1) {
        msg_err("%s error: pages_remaining %ld, should be %ld", func_ss, pages_remaining, allocs + 1);
        return false;
    }

    for (u64 i = 0; i < pages_remaining; i++) {
        if (i < allocs)
        ...
        if ((res = allocate_u64((heap)id, PAGESIZE)) != expect) {
            msg_err("%s error: remainder alloc returned 0x%lx, should be 0x%lx (iter %ld)",
...
static boolean alloc_align_test(heap h)
{
    ...
    for (u64 size = 1; pad(unaligned_base, size) + size <= unaligned_base + heap_size; size <<= 1) {
        u64 i = allocate_u64((heap)id, size);
        if (i != pad(unaligned_base, size)) {
            msg_err("%s error: unexpected allocation 0x%lx for size 0x%lx", func_ss, i, size);
            return false;
        }
...
int main(int argc, char **argv)
{
    ...
    heap rh = allocate_rangeheap(h,
				 ...
				 * 64 / N_RANGES);
    for (u64 order = 0; order <= MAX_PAGE_ORDER; order++) {
        curr_range = 0;
