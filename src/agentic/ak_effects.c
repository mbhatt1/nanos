...
/* Forward declaration for socket address lookup (implemented in
 * ak_posix_route.c) */
extern int ak_get_socket_bound_addr(int fd, struct sockaddr *addr,
                                    socklen_t *addrlen);
...
boolean ak_is_in_authorized_effect(void) { return ak_in_authorized_effect; }
...
__attribute__((weak)) ak_ctx_t *ak_ctx_current(void) { return current_ctx; }

__attribute__((weak)) void ak_ctx_set_current(ak_ctx_t *ctx) {
  current_ctx = ctx;
}

__attribute__((weak)) ak_ctx_t *ak_ctx_create(heap h,
...
__attribute__((weak)) void ak_ctx_destroy(heap h, ak_ctx_t *ctx) {
  if (!ctx)
    return;
...
__attribute__((weak)) void ak_ctx_set_mode(ak_ctx_t *ctx, ak_mode_t mode) {
  if (!ctx)
    return;
...
__attribute__((weak)) ak_mode_t ak_ctx_get_mode(ak_ctx_t *ctx) {
  if (!ctx)
    return AK_MODE_OFF;
  return ctx->mode;
}
...
__attribute__((weak)) boolean ak_ctx_boot_capsule_active(ak_ctx_t *ctx) {
  if (!ctx)
    return false;
  return ctx->boot_capsule_active && !ctx->boot_capsule_dropped;
}

__attribute__((weak)) void ak_ctx_drop_boot_capsule(ak_ctx_t *ctx) {
  if (!ctx)
    return;
...
__attribute__((weak)) u64 ak_trace_id_generate(ak_ctx_t *ctx) {
  u64 trace_id;
...
int ak_canonicalize_path(const char *path, char *out, u32 out_len,
  ...
  while (*src && pos < out_len - 1) {
    ...
    if (comp_len == 2 && src[0] == '.' && src[1] == '.') {
      ...
      if (min_pos == 0)
        min_pos = 1; /* At least stay at root */
      if (pos > min_pos) {
...
int ak_effect_from_openat(ak_effect_req_t *req, ak_ctx_t *ctx, int dirfd,
  ...
  int err =
      ak_fd_table_resolve_at(dirfd, path, resolved_path, sizeof(resolved_path));
  if (err < 0) {
    /*
...
int ak_effect_from_connect(ak_effect_req_t *req, ak_ctx_t *ctx,
  ...
  int err = ak_canonicalize_sockaddr(addr, addrlen, req->target, AK_MAX_TARGET);
  if (err != 0)
    return err;
...
static boolean ak_should_log_deny(ak_effect_op_t op) {
  ...
  if (now_ns - deny_rate_limiter.window_start_ns >
      AK_DENY_RATE_LIMIT_WINDOW_NS) {
    deny_rate_limiter.window_start_ns = now_ns;
    ak_memzero(deny_rate_limiter.count_per_op,
               sizeof(deny_rate_limiter.count_per_op));
  }
...
static void ak_log_deny(const ak_effect_req_t *req,
  ...
  rprintf("AK DENY %s %s missing %s. Fix: %s (trace=%lx)\n",
          ak_effect_op_name(req->op), req->target, decision->missing_cap,
          decision->suggested_snippet, decision->trace_id);
}
...
int ak_authorize_and_execute(ak_ctx_t *ctx, const ak_effect_req_t *req,
  ...
  if (ctx->mode == AK_MODE_RECORD) {
    ...
    if (!would_allow) {
      ...
      if (decision_out->suggested_snippet[0] == '\0') {
        ak_generate_suggestion(req, decision_out->suggested_snippet,
                               AK_MAX_SUGGEST);
      }
  ...
  if (ak_ctx_boot_capsule_active(ctx)) {
    ...
    runtime_strncpy(decision_out->detail,
                    "Operation denied during boot capsule phase",
                    AK_MAX_DETAIL);

    /* Update last_deny */
    ctx->last_deny.op = req->op;
    runtime_strncpy(ctx->last_deny.target, req->target, AK_MAX_TARGET);
    runtime_strncpy(ctx->last_deny.missing_cap, decision_out->missing_cap,
                    AK_MAX_CAPSTR);
  ...
  if (!ctx->policy) {
    ...
    runtime_strncpy(decision_out->detail, "No policy loaded - deny by default",
                    AK_MAX_DETAIL);
  ...
  if (decision_out->errno_equiv == 0) {
    decision_out->errno_equiv =
        ak_map_deny_to_errno(req->op, decision_out->reason_code);
  }
  ...
  if (decision_out->reason_code < 16)
    __sync_fetch_and_add(&global_stats.deny_reasons[decision_out->reason_code],
                         1);
...
void ak_generate_suggestion(const ak_effect_req_t *req, char *snippet,
  ...
  case 0x01: /* Filesystem */
    switch (req->op) {
    case AK_E_FS_OPEN:
      runtime_strncpy(snippet, "[[fs.allow]]\npath = \"", max_len);
      {
        u64 pos = runtime_strlen(snippet);
        u64 target_len = runtime_strlen(req->target);
        /* FIX(BUG-006): Correct bounds check - need 14 bytes for suffix + 1 for
         * null */
    ...
    case AK_E_FS_MKDIR:
      runtime_strncpy(snippet, "[[fs.allow]]\npath = \"", max_len);
      {
        u64 pos = runtime_strlen(snippet);
        u64 target_len = runtime_strlen(req->target);
        /* FIX(BUG-006): Correct bounds check - need 15 bytes for suffix + 1 for
         * null */
  ...
  case 0x02: /* Network */
    switch (req->op) {
    case AK_E_NET_CONNECT:
      runtime_strncpy(snippet, "[[net.allow]]\npattern = \"", max_len);
      {
        u64 pos = runtime_strlen(snippet);
        u64 target_len = runtime_strlen(req->target);
        /* FIX(BUG-006): Correct bounds check - need 17 bytes for suffix + 1 for
         * null */
    ...
    case AK_E_NET_DNS_RESOLVE:
      runtime_strncpy(snippet, "[[dns.allow]]\npattern = \"", max_len);
      {
        u64 pos = runtime_strlen(snippet);
        /* Skip "dns:" prefix */
  ...
  case 0x04: /* Agentic */
    switch (req->op) {
    case AK_E_TOOL_CALL:
      runtime_strncpy(snippet, "[[tools.allow]]\nname = \"", max_len);
      {
        u64 pos = runtime_strlen(snippet);
    ...
    case AK_E_INFER:
      runtime_strncpy(snippet, "[[inference.allow]]\nmodel = \"", max_len);
      {
        u64 pos = runtime_strlen(snippet);
    ...
    case AK_E_AGENT_SEND:
      runtime_strncpy(snippet, "[[ipc.allow]]\nagent = \"", max_len);
      {
        u64 pos = runtime_strlen(snippet);
