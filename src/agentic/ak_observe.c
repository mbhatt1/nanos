...
void ak_observe_init(heap h)
{
    ...
    decision_log.entries = ak_alloc_array_zero(h, ak_decision_log_entry_t,
...
static ak_metric_t *ak_metric_find_or_create(
    const char *name,
    const char *labels,
...
static int ak_format_prometheus_labels(const char *labels, char *buf, u64 max_len)
{
...
s64 ak_metrics_export_prometheus(char *buf, u64 max_len)
{
    ...
    for (u32 i = 0; i < metrics_state.metric_count && (u64)pos < max_len - 256; i++) {
        ak_metric_t *m = &metrics_state.metrics[i];

        char label_buf[512];
        int label_len = ak_format_prometheus_labels(m->labels, label_buf, sizeof(label_buf));
        ...
        case AK_METRIC_COUNTER:
            ...
            pos += bsnprintf(buf + pos, max_len - pos, "# TYPE %s counter\n",
                             m->name);
            if (label_len > 0) {
                pos += bsnprintf(buf + pos, max_len - pos, "%s%s %llu\n",
                                 m->name, label_buf, m->value.counter);
            } else {
        ...
        case AK_METRIC_GAUGE:
            ...
            if (label_len > 0) {
                pos += bsnprintf(buf + pos, max_len - pos, "%s%s %lld\n",
                                 m->name, label_buf, m->value.gauge);
            } else {
        ...
        case AK_METRIC_HISTOGRAM:
            ...
            pos += bsnprintf(buf + pos, max_len - pos, "# TYPE %s histogram\n",
                             m->name);

            /* Bucket entries */
            for (int b = 0; b < AK_HISTOGRAM_BUCKETS && (u64)pos < max_len - 128; b++) {
                u64 bucket_val = AK_LATENCY_BUCKETS[b];
                if (bucket_val == UINT64_MAX) {
                    pos += bsnprintf(buf + pos, max_len - pos,
                                     "%s_bucket{le=\"+Inf\"} %llu\n",
                                     m->name, m->value.histogram.buckets[b]);
                } else {
                    /* Convert to seconds for Prometheus */
                    double secs = (double)bucket_val / 1000000000.0;
                    pos += bsnprintf(buf + pos, max_len - pos,
                                     "%s_bucket{le=\"%.9f\"} %llu\n",
                                     m->name, secs, m->value.histogram.buckets[b]);
                }
            }
...
s64 ak_metrics_export_json(char *buf, u64 max_len)
{
    ...
    for (u32 i = 0; i < metrics_state.metric_count && (u64)pos < max_len - 256; i++) {
        ...
        const char *type_str;
        switch (m->type) {
        case AK_METRIC_COUNTER:   type_str = "counter"; break;
        case AK_METRIC_GAUGE:     type_str = "gauge"; break;
        case AK_METRIC_HISTOGRAM: type_str = "histogram"; break;
        ...
        case AK_METRIC_COUNTER:
            pos += bsnprintf(buf + pos, max_len - pos,
                             ",\"value\":%llu}", m->value.counter);
            break;
        case AK_METRIC_GAUGE:
            pos += bsnprintf(buf + pos, max_len - pos,
                             ",\"value\":%lld}", m->value.gauge);
            break;
        case AK_METRIC_HISTOGRAM:
            pos += bsnprintf(buf + pos, max_len - pos,
                             ",\"sum\":%llu,\"count\":%llu}",
    ...
    u64 latency_ns)
{
    ...
    bsnprintf(labels, sizeof(labels), "op=%s,allowed=%s",
              ak_effect_op_label(op),
              allowed ? "true" : "false");
    ...
    if (!allowed) {
        bsnprintf(labels, sizeof(labels), "op=%s,reason=%s",
                  ak_effect_op_label(op),
    ...
    ak_deny_reason_t reason)
{
    ...
    if (!allowed) {
        ak_trace_set_attribute(span, "effect.deny_reason", ak_deny_reason_label(reason));
    }
}
...
s64 ak_trace_export_otlp(char *buf, u64 max_len)
{
    ...
    while (span && (u64)pos < max_len - 512) {
        ...
        if (has_parent) {
            pos += bsnprintf(buf + pos, max_len - pos,
                             ",\"parentSpanId\":\"%s\"", parent_hex);
        }

        pos += bsnprintf(buf + pos, max_len - pos,
                         ",\"name\":\"%s\"", span->operation);

        /* Timing (in nanoseconds) */
        pos += bsnprintf(buf + pos, max_len - pos,
                         ",\"startTimeUnixNano\":%llu", span->start_ns);
        if (span->end_ns > 0) {
            pos += bsnprintf(buf + pos, max_len - pos,
                             ",\"endTimeUnixNano\":%llu", span->end_ns);
        }
    ...
    const ak_decision_t *decision)
{
    ...
    runtime_strncpy(entry->suggested_snippet, decision->suggested_snippet, AK_MAX_SUGGEST);
    ...
    if (decision_log.entry_count < decision_log.capacity) {
        ...
        decision_log.oldest_timestamp_ns = decision_log.entries[oldest_idx].timestamp_ns;
    }
    ...
    u64 max_len)
{
    ...
    for (u64 i = 0; i < decision_log.entry_count; i++) {
        u64 idx = (decision_log.head - 1 - i + decision_log.capacity) % decision_log.capacity;
        ak_decision_log_entry_t *entry = &decision_log.entries[idx];
    ...
    pos += bsnprintf(result + pos, max_len - pos,
                     "{\"trace_id\":\"%s\",\"sequence\":%llu",
                     trace_hex, found->sequence);

    pos += bsnprintf(result + pos, max_len - pos,
                     ",\"timestamp_ns\":%llu", found->timestamp_ns);
    ...
    pos += bsnprintf(result + pos, max_len - pos,
                     ",\"mode\":%d,\"boot_capsule_active\":%s",
                     found->mode, found->boot_capsule_active ? "true" : "false");
    ...
    if (found->missing_cap[0]) {
        pos += bsnprintf(result + pos, max_len - pos,
                         ",\"missing_cap\":\"%s\"", found->missing_cap);
    }

    pos += bsnprintf(result + pos, max_len - pos,
                     ",\"latency_ns\":%llu}", found->decision_latency_ns);
    ...
    u64 *count_out)
{
    ...
    ak_decision_log_entry_t **results = ak_alloc_array(h, ak_decision_log_entry_t *, match_count);
    ...
    u64 result_idx = 0;
    for (u64 i = 0; i < decision_log.entry_count && result_idx < match_count; i++) {
        u64 idx = (decision_log.head - 1 - i + decision_log.capacity) % decision_log.capacity;
        ak_decision_log_entry_t *entry = &decision_log.entries[idx];
...
s64 ak_decision_log_export_json(char *buf, u64 max_len, u64 limit)
{
    ...
    for (u64 i = 0; i < export_count && (u64)pos < max_len - 512; i++) {
        ...
        pos += bsnprintf(buf + pos, max_len - pos,
                         "{\"trace_id\":\"%s\",\"seq\":%llu,\"ts\":%llu",
                         trace_hex, entry->sequence, entry->timestamp_ns);
        ...
        pos += bsnprintf(buf + pos, max_len - pos,
                         ...
                         ak_deny_reason_label(entry->reason));

        pos += bsnprintf(buf + pos, max_len - pos,
                         ",\"latency_ns\":%llu}", entry->decision_latency_ns);
    }

    pos += bsnprintf(buf + pos, max_len - pos,
                     "],\"total\":%llu,\"exported\":%llu}",
...
void ak_decision_log_get_stats(ak_decision_log_stats_t *stats)
{
    ...
    stats->storage_bytes = decision_log.capacity * sizeof(ak_decision_log_entry_t);
