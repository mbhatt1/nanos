...
ak_budget_tracker_t *ak_budget_create(heap h, u8 *run_id, ak_policy_t *policy) {
  ...
  if (policy)
    runtime_memcpy(&tracker->limits, &policy->budgets,
                   sizeof(ak_budget_limits_t));
...
static u64 get_used(ak_budget_tracker_t *tracker, ak_resource_type_t type) {
  ...
  case AK_RESOURCE_TOKENS:
    return tracker->tokens_used;
  case AK_RESOURCE_CALLS:
    return tracker->calls_made;
  case AK_RESOURCE_INFERENCE_MS:
    return tracker->inference_ms_used;
  case AK_RESOURCE_FILE_BYTES:
    return tracker->file_bytes_used;
  case AK_RESOURCE_NETWORK_BYTES:
    return tracker->network_bytes_used;
  case AK_RESOURCE_HEAP_OBJECTS:
    return tracker->heap_objects_used;
  case AK_RESOURCE_HEAP_BYTES:
    return tracker->heap_bytes_used;
  default:
    return 0;
  }
}
...
static u64 get_reserved(ak_budget_tracker_t *tracker, ak_resource_type_t type) {
  switch (type) {
  case AK_RESOURCE_TOKENS:
    return tracker->tokens_reserved;
  case AK_RESOURCE_CALLS:
    return tracker->calls_reserved;
  case AK_RESOURCE_INFERENCE_MS:
    return tracker->inference_ms_reserved;
  default:
    return 0;
  }
}
...
static u64 get_limit(ak_budget_tracker_t *tracker, ak_resource_type_t type) {
  switch (type) {
  case AK_RESOURCE_TOKENS:
    return tracker->limits.tokens;
  case AK_RESOURCE_CALLS:
    return tracker->limits.calls;
  case AK_RESOURCE_INFERENCE_MS:
    return tracker->limits.inference_ms;
  case AK_RESOURCE_FILE_BYTES:
    return tracker->limits.file_bytes;
  case AK_RESOURCE_NETWORK_BYTES:
    return tracker->limits.network_bytes;
  case AK_RESOURCE_HEAP_OBJECTS:
    return tracker->limits.heap_objects;
  case AK_RESOURCE_HEAP_BYTES:
    return tracker->limits.heap_bytes;
  default:
    return 0;
  }
}

static void add_reserved(ak_budget_tracker_t *tracker, ak_resource_type_t type,
                         u64 amount) {
  switch (type) {
  case AK_RESOURCE_TOKENS:
...
static void sub_reserved(ak_budget_tracker_t *tracker, ak_resource_type_t type,
                         u64 amount) {
  switch (type) {
  case AK_RESOURCE_TOKENS:
...
static void add_used(ak_budget_tracker_t *tracker, ak_resource_type_t type,
                     u64 amount) {
  switch (type) {
  case AK_RESOURCE_TOKENS:
    tracker->tokens_used += amount;
    break;
  case AK_RESOURCE_CALLS:
    tracker->calls_made += amount;
    break;
  case AK_RESOURCE_INFERENCE_MS:
    tracker->inference_ms_used += amount;
    break;
  case AK_RESOURCE_FILE_BYTES:
    tracker->file_bytes_used += amount;
    break;
  case AK_RESOURCE_NETWORK_BYTES:
    tracker->network_bytes_used += amount;
    break;
  case AK_RESOURCE_HEAP_OBJECTS:
    tracker->heap_objects_used += amount;
    break;
  case AK_RESOURCE_HEAP_BYTES:
    tracker->heap_bytes_used += amount;
    break;
  default:
    break;
  }
...
void ak_budget_get_stats(ak_budget_tracker_t *tracker,
  ...
  stats->remaining.inference_ms =
      ak_budget_remaining(tracker, AK_RESOURCE_INFERENCE_MS);
  stats->remaining.file_bytes =
      ak_budget_remaining(tracker, AK_RESOURCE_FILE_BYTES);
  stats->remaining.network_bytes =
      ak_budget_remaining(tracker, AK_RESOURCE_NETWORK_BYTES);
  stats->remaining.spawn_count =
      tracker->limits.spawn_count - tracker->spawns_used;
  stats->remaining.heap_objects =
      ak_budget_remaining(tracker, AK_RESOURCE_HEAP_OBJECTS);
  stats->remaining.heap_bytes =
      ak_budget_remaining(tracker, AK_RESOURCE_HEAP_BYTES);
}
