...
#define AK_ASSERT_MSG(cond, msg) do { \
    if (unlikely(!(cond))) { \
        rprintf("AK ASSERT FAILED: %s\n  Message: %s\n  at %s:%d in %s()\n", \
...
#define AK_ASSERT_NOT_NULL(ptr) do { \
...
#define AK_CHECK_NOT_NULL(ptr, retval) do { \
    if (unlikely((ptr) == NULL)) { \
...
#define AK_CHECK_NOT_NULL_VOID(ptr) do { \
    if (unlikely((ptr) == NULL)) { \
        ak_error("NULL pointer: %s at %s:%d", #ptr, __func__, __LINE__); \
        return; \
    } \
...
#define AK_ASSERT_IN_RANGE(val, min, max) do { \
...
#define AK_ASSERT_INDEX(idx, len) do { \
    if (unlikely((idx) >= (len))) { \
        rprintf("AK INDEX FAILED: %s = %llu >= %s = %llu\n  at %s:%d in %s()\n", \
                #idx, (unsigned long long)(idx), #len, (unsigned long long)(len), \
                __FILE__, __LINE__, __func__); \
        halt("AK index assertion failed"); \
    } \
} while(0)

#define AK_ASSERT_POSITIVE(val) do { \
    if (unlikely((val) <= 0)) { \
...
#define AK_ASSERT_NON_NEGATIVE(val) do { \
    if (unlikely((val) < 0)) { \
...
#define AK_CHECK_IN_RANGE(val, min, max, retval) do { \
    if (unlikely((val) < (min) || (val) > (max))) { \
        ak_error("Value %s = %lld out of range [%lld, %lld] at %s:%d", \
                 #val, (long long)(val), (long long)(min), (long long)(max), \
                 __func__, __LINE__); \
        return (retval); \
    } \
...
#define AK_CHECK_INDEX(idx, len, retval) do { \
    if (unlikely((idx) >= (len))) { \
        ak_error("Index %s = %llu >= %s = %llu at %s:%d", \
                 #idx, (unsigned long long)(idx), #len, (unsigned long long)(len), \
                 __func__, __LINE__); \
        return (retval); \
    } \
...
#define AK_ASSERT_MAGIC(ptr, expected_magic) do { \
    if (unlikely((ptr)->magic != (expected_magic))) { \
        if ((ptr)->magic == AK_DEAD_MAGIC) { \
            rprintf("AK USE-AFTER-FREE: %s magic = 0x%x (DEAD)\n  at %s:%d in %s()\n", \
            ...
            rprintf("AK CORRUPT MAGIC: %s magic = 0x%x expected 0x%x\n  at %s:%d in %s()\n", \
                    #ptr, (ptr)->magic, expected_magic, __FILE__, __LINE__, __func__); \
...
#define AK_ASSERT_VALID_CTX(ctx) do { \
...
#define AK_CHECK_VALID_CTX(ctx, retval) do { \
...
#define AK_PRECONDITION(cond) do { \
    if (unlikely(!(cond))) { \
...
#define AK_POSTCONDITION(cond) do { \
    if (unlikely(!(cond))) { \
...
#define AK_ASSERT_STATE(current, expected) do { \
    if (unlikely((current) != (expected))) { \
        rprintf("AK STATE MISMATCH: %s = %d expected %d\n  at %s:%d in %s()\n", \
                #current, (int)(current), (int)(expected), \
                __FILE__, __LINE__, __func__); \
        halt("AK state assertion failed"); \
    } \
} while(0)

#define AK_ASSERT_STATE_IN(current, ...) do { \
    ...
    for (int _i = 0; _i < _num_states; _i++) { \
        if ((int)(current) == _valid_states[_i]) { _found = true; break; } \
    } \
    if (unlikely(!_found)) { \
        rprintf("AK INVALID STATE: %s = %d not in valid set\n  at %s:%d in %s()\n", \
...
#define AK_ASSERT_INV1_SYSCALL_GATE() do { \
...
#define AK_ASSERT_INV2_HAS_CAP(cap) do { \
...
#define AK_ASSERT_INV3_BUDGET(used, limit) do { \
...
#define AK_ASSERT_INV4_LOG_CHAIN(prev_hash, computed_hash) do { \
    if (unlikely(ak_memcmp((prev_hash), (computed_hash), AK_HASH_SIZE) != 0)) { \
...
#define AK_DEBUG_CHECK(cond, msg) do { \
    if (unlikely(!(cond))) { \
        ak_debug("DEBUG CHECK FAILED: %s - %s at %s:%d", #cond, msg, __func__, __LINE__); \
    } \
} while(0)
...
#define AK_STATIC_ASSERT(cond, msg) \
...
#define AK_ASSERT_ALIGNOF(type, align) \
    AK_STATIC_ASSERT(__alignof__(type) == (align), "alignof(" #type ") != " #align)
...
#define AK_DEFENSIVE_CLAMP(val, lo, hi) ({ \
    ...
    if (_v < _lo || _v > _hi) { \
        ak_warn("Clamping %s from %lld to [%lld, %lld] at %s:%d", \
                #val, (long long)_v, (long long)_lo, (long long)_hi, \
                __func__, __LINE__); \
