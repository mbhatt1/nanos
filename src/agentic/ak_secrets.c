...
void ak_secrets_init(heap h, ak_secrets_config_t *config) {
  ...
  secrets_state.file_path =
      config && config->file_path ? config->file_path : "/run/secrets";
  ...
  table_foreach(secrets_state.secrets, k, v) {
    ...
    if (buffer_length(key) >= ref_len) {
      ...
      if (runtime_strncmp(key_str, ref, ref_len) == 0 &&
          (buffer_length(key) == ref_len || byte(key, ref_len) == '\0')) {
        return (buffer)v;
      }
    }
  }
...
boolean ak_secret_check_cap(ak_capability_t *cap, const char *ref,
                            u64 ref_len) {
  if (!cap)
    return false;
...
const char *ak_secrets_last_error(void) { return secrets_state.last_error; }
