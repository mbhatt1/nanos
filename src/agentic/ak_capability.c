...
void ak_revocation_load_from_log(void) {
  ...
  for (u64 i = 0; i < count; i++) {
    ...
    spin_lock(&ak_cap_state.lock);
    boolean already_revoked =
        table_find(ak_cap_state.revocations, (void *)key) != NULL;

    if (!already_revoked) {
      /* Create new revocation entry */
      ak_revocation_entry_t *rev_entry =
          allocate(ak_cap_state.h, sizeof(*rev_entry));
      if (rev_entry && rev_entry != INVALID_ADDRESS) {
        runtime_memcpy(rev_entry->tid, tid, AK_TOKEN_ID_SIZE);
        rev_entry->revoked_ms = entry->ts_ms;
        /* Note: We don't have the original reason string available from the log
         * entry, only its hash. Store a placeholder reason for recovery. */
