...
ak_ctx_t *ak_ctx_create(heap h, ak_agent_context_t *agent) {
  ...
  ak_debug("ak_ctx_create: created context %p (id=%llu, agent=%p)", ctx,
           internal->context_id, agent);
...
ak_ctx_t *ak_ctx_current(void) { return tls_current_ctx; }
...
void ak_ctx_set_current(ak_ctx_t *ctx) {
  ...
  tls_current_ctx = ctx;
  ak_debug("ak_ctx_set_current: set TLS context to %p (id=%llu)", ctx,
           internal->context_id);
}
...
void ak_ctx_set_mode(ak_ctx_t *ctx, ak_mode_t mode) {
  ...
  ak_debug("ak_ctx_set_mode: context %p mode %d -> %d", ctx, old_mode, mode);
}
...
ak_mode_t ak_ctx_default_mode(void) { return AK_DEFAULT_MODE; }
...
void ak_ctx_drop_boot_capsule(ak_ctx_t *ctx) {
  ...
  if (ctx->boot_capsule_dropped) {
    ak_debug("ak_ctx_drop_boot_capsule: already dropped for context %p", ctx);
    return;
  }
  ...
  ak_debug("ak_ctx_drop_boot_capsule: context %p transitioned to "
           "deny-by-default (INV-DENY active)",
           ctx);
}
...
u64 ak_trace_id_generate(ak_ctx_t *ctx) {
  ...
  trace_id =
      ((internal->context_id & 0xFFFF) << 48) | (counter & 0xFFFFFFFFFFFF);
...
void ak_ctx_clear_last_deny(ak_ctx_t *ctx) {
  ...
  ak_debug("ak_ctx_clear_last_deny: cleared last denial for context %p", ctx);
}
...
void ak_ctx_set_policy(ak_ctx_t *ctx, struct ak_policy_v2 *policy) {
  ...
  ak_debug("ak_ctx_set_policy: context %p policy set to %p", ctx, policy);
}
