...
static boolean local_isdigit(char c)
{
    return c >= '0' && c <= '9';
}
...
static ak_tool_def_t *allocate_tool(void)
{
    ...
    /* Use new slot */
    ak_tool_def_t *tool = &registry_state.tool_pool[registry_state.tool_pool_used];
...
int ak_tool_register(const ak_tool_def_t *def)
{
    ...
    if (def->mock_enabled && def->mock_response) {
        tool->mock_response = allocate_buffer(registry_state.h,
                                               buffer_length(def->mock_response));
        if (tool->mock_response != INVALID_ADDRESS) {
            buffer_write(tool->mock_response,
                        buffer_ref(def->mock_response, 0),
                        buffer_length(def->mock_response));
        }
    } else {
...
void ak_tool_list_free(heap h, ak_tool_list_t *list)
{
    ...
    if (list->tools)
        deallocate(alloc_heap, list->tools, list->capacity * sizeof(ak_tool_def_t *));
                           ...
                           buffer args, buffer *result, u64 timeout_ms)
{
    ...
    if (tool->mock_enabled && tool->mock_response) {
        ...
        if (result) {
            *result = allocate_buffer(registry_state.h,
                                       buffer_length(tool->mock_response));
            if (*result != INVALID_ADDRESS) {
                buffer_write(*result,
                            buffer_ref(tool->mock_response, 0),
                            buffer_length(tool->mock_response));
            }
        }
...
static s64 composite_handler(ak_ctx_t *ctx, buffer args, buffer *result)
{
    ...
    for (u32 i = 0; i < comp->chain_length; i++) {
        ...
        const char *tool_version = comp->chain[i].version[0] ? comp->chain[i].version : NULL;

        /* Invoke tool in chain */
        int err = ak_tool_invoke(ctx, tool_name, tool_version,
                    ...
                    u32 count)
{
    ...
    ak_composite_tool_t *comp = allocate(registry_state.h,
                                          sizeof(ak_composite_tool_t));
    if (!comp)
        return -ENOMEM;
    ...
    local_strncpy(comp->version, version ? version : "1.0.0", sizeof(comp->version));
    ...
    for (u32 i = 0; i < count; i++) {
        ...
        local_strncpy(comp->chain[i].name, tool_names[i], sizeof(comp->chain[i].name));
...
int ak_tool_compose_ex(const ak_composite_tool_t *composite)
{
    ...
    for (u32 i = 0; i < composite->chain_length; i++) {
        ...
        tool_versions[i] = composite->chain[i].version[0] ? composite->chain[i].version : NULL;
    }

    return ak_tool_compose(composite->name, composite->version,
                           tool_names, tool_versions, composite->chain_length);
}
