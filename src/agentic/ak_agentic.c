...
int ak_tool_handler_register(const char *name, const char *version,
  ...
  ak_native_tool_t *tool =
      &ak_agentic_state.native_tools[ak_agentic_state.native_tool_count];
...
ak_tool_handler_t ak_tool_handler_lookup(const char *name,
                                         const char *version) {
  if (!ak_agentic_state.initialized || !name)
    return NULL;
...
int ak_handle_tool_call(ak_ctx_t *ctx, const char *tool_name,
  ...
  ak_effect_req_t req;
  int err =
      ak_effect_from_tool_call(&req, ctx, tool_name, version, args, args_len);
  if (err != 0) {
  ...
  if (err != 0 || !decision.allow) {
    ...
    ak_debug("ak_agentic: tool call '%s' denied: %s", tool_name,
             decision.detail);
  ...
  if (!ak_budget_check_tool(ctx)) {
    ...
    local_strncpy(ctx->last_deny.missing_cap, "budget.tool_calls",
                  AK_MAX_CAPSTR);
    ...
    if (reg_tool) {
      ...
      if (wasm_tool && ctx->agent) {
        ...
        ak_response_t *response =
            ak_wasm_execute_tool(ctx->agent, tool_name, args_buf, NULL);
...
int ak_handle_wasm_invoke(ak_ctx_t *ctx, const char *module,
  ...
  ak_effect_req_t req;
  int err =
      ak_effect_from_wasm_invoke(&req, ctx, module, function, args, args_len);
  if (err != 0) {
  ...
  if (err != 0 || !decision.allow) {
    ...
    ak_debug("ak_agentic: wasm invoke '%s:%s' denied: %s", module, function,
             decision.detail);
  ...
  if (!ak_budget_check_wasm(ctx)) {
    ...
    ctx->last_deny.reason = AK_DENY_BUDGET_EXCEEDED;
    ctx->last_deny.errno_equiv = ENOSPC;
    local_strncpy(ctx->last_deny.missing_cap, "budget.wasm_calls",
                  AK_MAX_CAPSTR);
    local_strncpy(ctx->last_deny.suggested_snippet,
                  "[budgets]\nwasm_calls = 2000  # Increase limit",
                  AK_MAX_SUGGEST);

    ak_debug("ak_agentic: wasm invoke '%s:%s' budget exceeded", module,
             function);
  ...
  if (ctx->agent) {
    ...
    if (tool && tool->module == wasm_module) {
      ...
      if (args && args_len > 0) {
         ...
         * If allocation fails, return error instead of continuing with invalid
         * buffer.
         */
        if (!args_buf || args_buf == INVALID_ADDRESS) {
          ak_agentic_state.stats.wasm_invokes_failed++;
      ...
      ak_wasm_exec_ctx_t *exec_ctx =
          ak_wasm_exec_create(ak_agentic_state.h, ctx->agent, tool, NULL);
      ...
      if (err == 0 && exec_ctx->state == AK_WASM_STATE_COMPLETED &&
          exec_ctx->output) {
        ...
        err =
            exec_ctx->result_code != 0 ? exec_ctx->result_code : AK_E_WASM_TRAP;
      }
...
int ak_wasm_hostcall_check(ak_ctx_t *ctx, const char *module,
  ...
  if (ctx->policy) {
    ...
    if (!allowed) {
      ...
      /* Format target */
      /* FIX(BUG-002): Use cumulative length tracking with proper bounds
       * validation */
      int len = 0;
      int remaining = AK_MAX_TARGET - 1; /* Reserve 1 for null terminator */
      ...
      ak_debug("ak_agentic: hostcall '%s' denied for module '%s'", hostcall,
               module);
...
int ak_handle_infer(ak_ctx_t *ctx, const char *model, const char *version,
  ...
  if (ctx->agent) {
    ...
    ak_inference_response_t *inf_res =
        ak_inference_complete(ctx->agent, &inf_req, NULL);
    ...
    if (inf_res && inf_res->success && inf_res->content) {
      ...
      ak_agentic_state.stats.infer_tokens_out +=
