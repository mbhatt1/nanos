...
static const char *ak_record_category_name(ak_effect_op_t op)
{
    ...
    case 0x03: return "process";
    case 0x04: return "agentic";
...
{
    switch (op) {
    case AK_E_FS_OPEN:         return "open";
    case AK_E_FS_UNLINK:       return "unlink";
    case AK_E_FS_RENAME:       return "rename";
    case AK_E_FS_MKDIR:        return "mkdir";
    case AK_E_FS_RMDIR:        return "rmdir";
    case AK_E_FS_STAT:         return "stat";
    case AK_E_NET_CONNECT:     return "connect";
    case AK_E_NET_DNS_RESOLVE: return "dns_resolve";
    case AK_E_NET_BIND:        return "bind";
    case AK_E_NET_LISTEN:      return "listen";
    case AK_E_NET_ACCEPT:      return "accept";
    case AK_E_PROC_SPAWN:      return "spawn";
    case AK_E_PROC_SIGNAL:     return "signal";
    case AK_E_PROC_WAIT:       return "wait";
    case AK_E_TOOL_CALL:       return "tool_call";
    case AK_E_WASM_INVOKE:     return "wasm_invoke";
    case AK_E_INFER:           return "inference";
    default:                   return "unknown";
    }
}
                                    ...
                                    const ak_effect_req_t *req)
{
    ...
    case 0x01:  /* Filesystem */
        {
            ...
            if (is_write) {
                pos += runtime_strncpy(out + pos, ",\"write\":true}", max_len - pos) ? 14 : 0;
            } else {
                pos += runtime_strncpy(out + pos, ",\"read\":true}", max_len - pos) ? 13 : 0;
            }
        }
        break;
    ...
    case 0x02:  /* Network */
        if (req->op == AK_E_NET_DNS_RESOLVE) {
            ...
            pos += runtime_strncpy(out + pos, "{\"pattern\":", max_len - pos) ? 11 : 0;
            pos += json_write_escaped_string(out + pos, max_len - pos, host);
            pos += runtime_strncpy(out + pos, ",\"resolve\":true}", max_len - pos) ? 16 : 0;
        } else {
            /* Network connection */
            pos += runtime_strncpy(out + pos, "{\"pattern\":", max_len - pos) ? 11 : 0;
            pos += json_write_escaped_string(out + pos, max_len - pos, req->target);
            ...
            case AK_E_NET_CONNECT:
                pos += runtime_strncpy(out + pos, ",\"connect\":true}", max_len - pos) ? 16 : 0;
                break;
            case AK_E_NET_BIND:
                pos += runtime_strncpy(out + pos, ",\"bind\":true}", max_len - pos) ? 13 : 0;
                break;
            case AK_E_NET_LISTEN:
                pos += runtime_strncpy(out + pos, ",\"listen\":true}", max_len - pos) ? 15 : 0;
                break;
            default:
...
int ak_record_effect(ak_record_state_t *state, const ak_effect_req_t *req)
{
    ...
    ak_debug("ak_record: recorded effect %s target=%s (total=%u)",
             ak_record_effect_type_name(req->op), req->target, state->unique_count);
                                    ...
                                    char *out, u64 out_len)
{
    ...
    pos += runtime_strncpy(out + pos, "\"effects_recorded\":", remaining) ? 19 : 0;
    remaining = (int)out_len - 1 - pos;
    ...
    for (int i = 0; i < AK_RECORD_HASH_SIZE; i++) {
        ...
        while (entry) {
            ...
            if (cat == 0x01) has_fs = true;
            else if (cat == 0x02) has_net = true;
            else if (cat == 0x04) has_agentic = true;
            entry = entry->next;
        }
    }
    ...
    if (has_fs && remaining > 50) {
        pos += runtime_strncpy(out + pos, ",\"fs\":{\"allow\":[", remaining) ? 16 : 0;
        remaining = (int)out_len - 1 - pos;
    ...
    if (has_net && remaining > 50) {
        pos += runtime_strncpy(out + pos, ",\"net\":{\"allow\":[", remaining) ? 17 : 0;
        remaining = (int)out_len - 1 - pos;
    ...
    if (has_agentic && remaining > 50) {
        ...
        if (has_tools && remaining > 40) {
            pos += runtime_strncpy(out + pos, ",\"tools\":{\"allow\":[", remaining) ? 19 : 0;
            remaining = (int)out_len - 1 - pos;
        ...
        if (has_inference && remaining > 40) {
            pos += runtime_strncpy(out + pos, ",\"inference\":{\"allow\":[", remaining) ? 23 : 0;
            remaining = (int)out_len - 1 - pos;
                                         ...
                                         char *out, u64 out_len)
{
    ...
    for (int i = 0; i < AK_RECORD_HASH_SIZE && remaining > 50; i++) {
        ...
        while (entry && remaining > 50) {
            if (((entry->op >> 8) & 0xFF) == 0x01) {
                boolean is_write = (entry->op == AK_E_FS_UNLINK ||
                                   entry->op == AK_E_FS_RENAME ||
                                   entry->op == AK_E_FS_MKDIR ||
                                   entry->op == AK_E_FS_RMDIR);
                ...
                if (is_write) {
                    ...
                    pos += runtime_strncpy(out + pos, "read = true\n\n", remaining) ? 13 : 0;
                }
                remaining = (int)out_len - 1 - pos;
            }
            entry = entry->next;
        ...
        while (entry && remaining > 50) {
            if (((entry->op >> 8) & 0xFF) == 0x02) {
                if (entry->op == AK_E_NET_DNS_RESOLVE) {
                    pos += runtime_strncpy(out + pos, "[[dns.allow]]\n", remaining) ? 14 : 0;
                    remaining = (int)out_len - 1 - pos;
                    ...
                    pos += runtime_strncpy(out + pos, "[[net.allow]]\n", remaining) ? 14 : 0;
                    remaining = (int)out_len - 1 - pos;
    ...
    for (int i = 0; i < AK_RECORD_HASH_SIZE && remaining > 40; i++) {
        ...
        while (entry && remaining > 40) {
            if (entry->op == AK_E_TOOL_CALL) {
                pos += runtime_strncpy(out + pos, "[[tools.allow]]\n", remaining) ? 16 : 0;
                remaining = (int)out_len - 1 - pos;
