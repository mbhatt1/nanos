...
int ak_fd_table_register(int fd, const char *path, boolean is_dir)
{
    ...
    if (entry) {
        ...
        spin_unlock(&fd_table_lock);
        ak_debug("ak_fd_table: updated fd=%d path=%s dir=%d",
                 fd, entry->path, is_dir);
        return 0;
    }
    ...
    ak_debug("ak_fd_table: registered fd=%d path=%s dir=%d",
             fd, entry->path, is_dir);
    return 0;
}
...
int ak_fd_table_resolve_at(int dirfd, const char *path, char *out, u64 max_len)
{
    ...
    if (!entry->is_directory) {
        fd_stats.resolve_errors++;
        spin_unlock(&fd_table_lock);
        ak_debug("ak_fd_table: resolve_at failed - fd=%d is not a directory", dirfd);
        return -ENOTDIR;
    }
...
void ak_fd_table_dump(void)
{
    ...
    rprintf("ak_fd_table: dump (entries=%d, peak=%d)\n",
            fd_stats.current_entries, fd_stats.peak_entries);
    rprintf("  cwd: %s\n", current_cwd);
    ...
    for (u32 i = 0; i < AK_FD_TABLE_SIZE; i++) {
        if (fd_table[i].valid) {
            rprintf("  fd=%d dir=%d path=%s\n",
                    fd_table[i].fd,
                    fd_table[i].is_directory,
                    fd_table[i].path);
        }
    }

    rprintf("  stats: reg=%lu unreg=%lu lookup=%lu (hit=%lu miss=%lu) resolve=%lu err=%lu\n",
            fd_stats.registers, fd_stats.unregisters, fd_stats.lookups,
            fd_stats.lookup_hits, fd_stats.lookup_misses,
            fd_stats.resolves, fd_stats.resolve_errors);
