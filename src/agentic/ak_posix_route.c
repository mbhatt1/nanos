 ...
 *   Callers should use the wildcard pattern "ip:*:*" for unbound/unknown
 * sockets.
 *
 * Integration Note:
 ...
 */
int ak_get_socket_bound_addr(int fd, struct sockaddr *addr,
                             socklen_t *addrlen) {
...
sysreturn ak_route_renameat(int olddirfd, const char *oldpath, int newdirfd,
  ...
  int err =
      ak_resolve_path_at(olddirfd, oldpath, resolved_old, sizeof(resolved_old));
  if (err < 0)
    return err;

  err =
      ak_resolve_path_at(newdirfd, newpath, resolved_new, sizeof(resolved_new));
  if (err < 0)
    return err;
...
sysreturn ak_route_listen(int fd, int backlog) {
   ...
   * If canonicalization fails, use wildcard "ip:*:*" for unbound/unknown
   * sockets.
   */
...
void ak_route_register_fd_from_openat(int fd, int dirfd, const char *path,
  ...
  if (path[0] == '/') {
    ...
    /* Relative to dirfd */
    err = ak_fd_table_resolve_at(dirfd, path, resolved_path,
                                 sizeof(resolved_path));
  }

  if (err < 0) {
    ak_debug("ak_route_register_fd_from_openat: resolve failed fd=%d err=%d",
             fd, err);
    return;
  }
  ...
  if (err < 0) {
    ak_debug("ak_route_register_fd_from_openat: register failed fd=%d err=%d",
             fd, err);
  }
}
