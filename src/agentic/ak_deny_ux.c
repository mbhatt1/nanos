...
boolean ak_deny_log_entry(const ak_effect_req_t *req,
  ...
  rprintf("AK DENY %s %s missing %s. Fix: %s (trace=%lx)\n",
          ak_effect_op_to_string(req->op), req->target, decision->missing_cap,
          decision->suggested_snippet, decision->trace_id);

  return true;
}

u64 ak_deny_log_get_suppressed(void) { return log_state.suppressed_count; }
...
void ak_trace_ring_init(heap h) {
  ...
  trace_ring.entries =
      allocate_zero(h, trace_ring.capacity * sizeof(ak_trace_entry_t));
...
void ak_trace_ring_push(const ak_effect_req_t *req,
  ...
  /* FIX(BUG-003): Clamp target_len to sizeof(entry->target)-1 to prevent buffer
   * overflow */
...
sysreturn ak_sys_trace_ring_read(u8 *buf, u64 buf_len, u64 *offset) {
  ...
  u32 count =
      ak_trace_ring_read((ak_trace_entry_t *)buf, max_entries, &local_offset);
...
void ak_record_init(heap h) {
  ...
  record_state.denials =
      allocate_zero(h, record_state.capacity * sizeof(*record_state.denials));
...
u32 ak_record_count(void) { return record_state.count; }

u64 ak_record_export(char *buf, u64 buf_len) {
  /* FIX(BUG-010): Require minimum buffer size to prevent underflow in remaining
   * calculations */
  ...
  /* FIX(BUG-010): Use remaining to prevent underflow; require 128 bytes
   * headroom */
...
int ak_suggest_for_fs(const ak_effect_req_t *req, char *buf, u32 buf_len) {
  ...
  boolean is_write = (req->op == AK_E_FS_UNLINK || req->op == AK_E_FS_RENAME ||
...
int ak_suggest_for_net(const ak_effect_req_t *req, char *buf, u32 buf_len) {
  ...
  if (req->op == AK_E_NET_DNS_RESOLVE) {
    /* DNS suggestion */
    pos +=
        runtime_strncpy(buf + pos, "[[dns.allow]]\n", buf_len - pos) ? 14 : 0;
    ...
    pos +=
        runtime_strncpy(buf + pos, "[[net.allow]]\n", buf_len - pos) ? 14 : 0;
    pos += runtime_strncpy(buf + pos, "pattern = \"", buf_len - pos) ? 11 : 0;
    ...
    case AK_E_NET_CONNECT:
      pos +=
          runtime_strncpy(buf + pos, "connect = true", buf_len - pos) ? 14 : 0;
      break;
    case AK_E_NET_BIND:
    ...
    case AK_E_NET_LISTEN:
      pos +=
          runtime_strncpy(buf + pos, "listen = true", buf_len - pos) ? 13 : 0;
      break;
    default:
      break;
    }
...
int ak_suggest_for_tool(const ak_effect_req_t *req, char *buf, u32 buf_len) {
  ...
  pos +=
      runtime_strncpy(buf + pos, "[[tools.allow]]\n", buf_len - pos) ? 16 : 0;
