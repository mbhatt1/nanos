...
s64 ak_ipc_recv(ak_ipc_channel_t *ch, ak_request_t **req_out,
                ak_response_t **res_out) {
  return ak_ipc_recv_timeout(ch, req_out, res_out, 0);
}

s64 ak_ipc_recv_timeout(ak_ipc_channel_t *ch, ak_request_t **req_out,
                        ak_response_t **res_out, u32 timeout_ms) {
  ...
  if (seq >= tracker->window_base + SEQ_WINDOW_SIZE) {
    ...
    if (shift >= SEQ_WINDOW_SIZE) {
      ...
      if (bytes_to_shift > 0) {
        ...
        runtime_memset(tracker->seen_bitmap + SEQ_WINDOW_SIZE / 8 -
                           bytes_to_shift,
                       0, bytes_to_shift);
      }
    }
...
buffer ak_ipc_serialize_batch_response(heap h, ak_response_t **responses,
                                       u64 count) {
...
ak_response_t *ak_ipc_error_response(heap h, u8 *pid, u8 *run_id, u64 seq,
                                     s64 error_code, const char *message) {
...
const char *ak_ipc_error_message(s64 error_code) {
  ...
  case AK_E_CAP_SCOPE:
    return "capability scope insufficient";
  case AK_E_CAP_RATE:
    return "rate limit exceeded";
  case AK_E_CAP_RUN_MISMATCH:
  ...
  case AK_E_SCHEMA_INVALID:
    return "schema validation failed";
  case AK_E_POLICY_DENIED:
    return "policy denied";
  case AK_E_BUDGET_EXCEEDED:
    return "budget exceeded";
  case AK_E_IPC_INVALID:
    return "invalid IPC frame";
  case AK_E_TAINT:
    return "taint violation";
  case AK_E_LOG_CORRUPT:
...
u16 ak_string_to_op(const char *str) {
  ...
  if (ak_strcmp(str, "READ") == 0)
    return AK_SYS_READ;
  if (ak_strcmp(str, "ALLOC") == 0)
    return AK_SYS_ALLOC;
  if (ak_strcmp(str, "WRITE") == 0)
    return AK_SYS_WRITE;
  if (ak_strcmp(str, "DELETE") == 0)
    return AK_SYS_DELETE;
  if (ak_strcmp(str, "QUERY") == 0)
    return AK_SYS_QUERY;
  if (ak_strcmp(str, "BATCH") == 0)
    return AK_SYS_BATCH;
  if (ak_strcmp(str, "COMMIT") == 0)
    return AK_SYS_COMMIT;
  if (ak_strcmp(str, "CALL") == 0)
    return AK_SYS_CALL;
  if (ak_strcmp(str, "SPAWN") == 0)
    return AK_SYS_SPAWN;
  if (ak_strcmp(str, "SEND") == 0)
    return AK_SYS_SEND;
  if (ak_strcmp(str, "RECV") == 0)
    return AK_SYS_RECV;
  if (ak_strcmp(str, "RESPOND") == 0)
    return AK_SYS_RESPOND;
  if (ak_strcmp(str, "ASSERT") == 0)
    return AK_SYS_ASSERT;
  if (ak_strcmp(str, "INFERENCE") == 0)
    return AK_SYS_INFERENCE;
