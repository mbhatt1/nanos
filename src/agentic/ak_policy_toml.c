...
/* Set parser error */
static void toml_set_error(ak_toml_parser_t *p, ak_toml_error_t err,
                           const char *msg) {
  p->error = err;
  snprintf(p->error_msg, sizeof(p->error_msg), "Line %d, col %d: %s", p->line,
           p->col, msg);
}
...
static bool toml_parse_bare_key(ak_toml_parser_t *p, char *out,
                                size_t max_len) {
  size_t i = 0;
...
static bool toml_parse_section(ak_toml_parser_t *p) {
  ...
  while (i > 0 && TOML_IS_WS(p->section[i - 1]))
    i--;
  p->section[i] = '\0';
...
static bool toml_parse_keyval(ak_toml_parser_t *p) {
  ...
  if (toml_peek(p) == '"' || toml_peek(p) == '\'') {
    if (!toml_parse_string(p, key, sizeof(key)))
      return false;
  } else {
  ...
  if (c == '"' || c == '\'') {
    ...
    if (!toml_parse_string(p, strval, sizeof(strval)))
      return false;
  } else if (c == '-' || c == '+' || TOML_IS_DIGIT(c)) {
    val_type = AK_TOML_TYPE_INTEGER;
    if (!toml_parse_integer(p, &intval))
      return false;
  } else if (c == 't' || c == 'f') {
    val_type = AK_TOML_TYPE_BOOLEAN;
    if (!toml_parse_boolean(p, &boolval))
      return false;
  } else if (c == '[') {
    ...
    toml_set_error(p, AK_TOML_ERR_SYNTAX,
                   "Arrays not supported in key-value context");
    return false;
  } else {
  ...
  if (strcmp(p->section, "fs.allow") == 0) {
    ...
    if (strcmp(key, "path") == 0 && val_type == AK_TOML_TYPE_STRING) {
      ...
      strncpy(pol->fs_rules[idx].path, strval,
              sizeof(pol->fs_rules[idx].path) - 1);
      if (pol->fs_rule_count == 0)
        pol->fs_rule_count = 1;
  ...
  if (strcmp(p->section, "net.allow") == 0) {
    ...
    if (strcmp(key, "pattern") == 0 && val_type == AK_TOML_TYPE_STRING) {
      ...
      strncpy(pol->net_rules[idx].pattern, strval,
              sizeof(pol->net_rules[idx].pattern) - 1);
      if (pol->net_rule_count == 0)
        pol->net_rule_count = 1;
  ...
  if (strcmp(p->section, "dns.allow") == 0) {
    ...
    if (strcmp(key, "pattern") == 0 && val_type == AK_TOML_TYPE_STRING) {
      ...
      strncpy(pol->dns_rules[idx].pattern, strval,
              sizeof(pol->dns_rules[idx].pattern) - 1);
      pol->dns_rules[idx].allow = true; /* Default for dns.allow */
      if (pol->dns_rule_count == 0)
        pol->dns_rule_count = 1;
    }
    return true;
  }
  ...
  if (strcmp(p->section, "tools.allow") == 0) {
    ...
    if (strcmp(key, "name") == 0 && val_type == AK_TOML_TYPE_STRING) {
      ...
      strncpy(pol->tool_allow_rules[idx].name, strval,
              sizeof(pol->tool_allow_rules[idx].name) - 1);
      pol->tool_allow_rules[idx].allow = true;
      if (pol->tool_allow_count == 0)
        pol->tool_allow_count = 1;
    } else if (strcmp(key, "version") == 0 && val_type == AK_TOML_TYPE_STRING) {
      strncpy(pol->tool_allow_rules[idx].version, strval,
              sizeof(pol->tool_allow_rules[idx].version) - 1);
    }
    return true;
  }
  ...
  if (strcmp(p->section, "tools.deny") == 0) {
    ...
    if (strcmp(key, "name") == 0 && val_type == AK_TOML_TYPE_STRING) {
      ...
      strncpy(pol->tool_deny_rules[idx].name, strval,
              sizeof(pol->tool_deny_rules[idx].name) - 1);
      pol->tool_deny_rules[idx].allow = false;
      if (pol->tool_deny_count == 0)
        pol->tool_deny_count = 1;
    }
    return true;
  }
  ...
  if (strcmp(p->section, "wasm.allow") == 0) {
    ...
    if (strcmp(key, "module") == 0 && val_type == AK_TOML_TYPE_STRING) {
      ...
      strncpy(pol->wasm_rules[idx].module, strval,
              sizeof(pol->wasm_rules[idx].module) - 1);
      if (pol->wasm_rule_count == 0)
        pol->wasm_rule_count = 1;
    }
    return true;
  }
  ...
  if (strcmp(p->section, "inference.allow") == 0) {
    ...
    if (strcmp(key, "model") == 0 && val_type == AK_TOML_TYPE_STRING) {
      ...
      strncpy(pol->infer_rules[idx].model, strval,
              sizeof(pol->infer_rules[idx].model) - 1);
      pol->infer_rules[idx].max_tokens = 100000; /* Default */
      if (pol->infer_rule_count == 0)
        pol->infer_rule_count = 1;
  ...
  if (strcmp(p->section, "budgets") == 0) {
    ...
    } else if (strcmp(key, "wall_time_ms") == 0 &&
               val_type == AK_TOML_TYPE_INTEGER) {
...
ak_toml_error_t ak_toml_parse(const char *toml, size_t toml_len,
  ...
  ak_toml_parser_t parser = {.input = toml,
                             .input_len = toml_len,
                             .pos = 0,
                             .line = 1,
                             .col = 1,
  ...
  if (policy->tool_allow_count > 0 || policy->tool_deny_count > 0) {
    ...
    if (policy->tool_allow_count > 0) {
      ...
      for (int i = 0; i < policy->tool_allow_count; i++) {
        if (i > 0 && !json_append(&out, &remain, ", "))
          return AK_TOML_ERR_OVERFLOW;
        if (!json_append_quoted(&out, &remain,
                                policy->tool_allow_rules[i].name))
          return AK_TOML_ERR_OVERFLOW;
      }
      if (!json_append(&out, &remain, "]"))
        return AK_TOML_ERR_OVERFLOW;
...
void ak_toml_policy_print(const ak_toml_policy_t *policy) {
  ...
  for (int i = 0; i < policy->net_rule_count; i++) {
    printf("    [%d] pattern=%s connect=%d bind=%d listen=%d\n", i,
           policy->net_rules[i].pattern, policy->net_rules[i].connect,
           policy->net_rules[i].bind, policy->net_rules[i].listen);
  }
