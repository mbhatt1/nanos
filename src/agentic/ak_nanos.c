 ...
 */
static inline boolean validate_user_memory(const void *addr, u64 len,
                                           boolean write) {
  (void)write;
 ...
 */
void ak_nanos_shutdown(void) { ak_shutdown(); }
 ...
 */
void ak_set_context(ak_agent_context_t *ctx) { current_context = ctx; }

/*
 * Get agent context for current thread.
 */
ak_agent_context_t *ak_get_context(void) { return current_context; }
 ...
 */
__attribute__((unused)) static s64 ak_nanos_dispatch(u64 syscall_num, u64 arg0,
                                                     u64 arg1, u64 arg2,
                                                     u64 arg3, u64 arg4,
                                                     u64 arg5) {
...
/* Direct heap write */
s64 ak_sys_write(u64 ptr, u8 *patch, u64 patch_len, u64 expected_version,
                 u64 *new_version_out) {
  ak_agent_context_t *ctx = current_context;
  if (!ctx)
    return -EPERM;
 ...
 */
s64 ak_verify_audit_log(void) { return ak_audit_verify(); }

/*
 * Get audit statistics.
 */
void ak_get_audit_stats(ak_audit_stats_t *stats) { ak_audit_get_stats(stats); }
...
s64 ak_create_initial_caps(ak_agent_context_t *ctx) {
  ...
  ak_grant_capability(ctx, AK_CAP_HEAP, "*", heap_methods, 3600000, 1000,
                      60000); /* 1 hour, 1000 ops/min */

  /* Grant limited tool capability */
  const char *tool_methods[] = {"invoke", NULL};
  ak_grant_capability(ctx, AK_CAP_TOOL, "file_read", tool_methods, 3600000, 100,
                      60000);
