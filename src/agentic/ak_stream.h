...
typedef void (*ak_stream_on_chunk_t)(struct ak_stream_session *session,
                                     struct ak_stream_chunk *chunk, void *ctx);

/* Callback invoked when stream closes */
typedef void (*ak_stream_on_close_t)(struct ak_stream_session *session,
                                     ak_stream_state_t reason, void *ctx);

/* Callback for token streaming (LLM) */
typedef void (*ak_stream_on_token_t)(struct ak_stream_session *session,
 ...
 */
s64 ak_stream_send_chunk(ak_stream_session_t *session, const u8 *data, u64 len);
...
#define AK_E_STREAM_BACKPRESSURE                                               \
  (-4708) /* Consumer slow, backpressure active */
