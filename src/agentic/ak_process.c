 ...
 */
static boolean fs_rules_subsumed(ak_fs_rule_v2_t *parent_rules,
                                 ak_fs_rule_v2_t *child_rules) {
  ...
  for (ak_fs_rule_v2_t *child = child_rules; child; child = child->next) {
    ...
    for (ak_fs_rule_v2_t *parent = parent_rules; parent;
         parent = parent->next) {
 ...
 */
static boolean net_rules_subsumed(ak_net_rule_v2_t *parent_rules,
                                  ak_net_rule_v2_t *child_rules) {
  if (!child_rules)
    return true;
  ...
  for (ak_net_rule_v2_t *child = child_rules; child; child = child->next) {
    ...
    for (ak_net_rule_v2_t *parent = parent_rules; parent;
         parent = parent->next) {
      if (!ak_pattern_match(parent->pattern, child->pattern))
        continue;
 ...
 */
static boolean tool_rules_subsumed(ak_tool_rule_v2_t *parent_rules,
                                   ak_tool_rule_v2_t *child_rules) {
  if (!child_rules)
    return true;
  ...
  for (ak_tool_rule_v2_t *child = child_rules; child; child = child->next) {
    ...
    for (ak_tool_rule_v2_t *parent = parent_rules; parent;
         parent = parent->next) {
      if (!parent->allow)
 ...
 */
static boolean budgets_subsumed(const ak_budgets_v2_t *parent,
                                const ak_budgets_v2_t *child) {
  /* CPU budget */
  if (parent->cpu_ns > 0 &&
      (child->cpu_ns == 0 || child->cpu_ns > parent->cpu_ns))
    return false;

  /* Wall time budget */
  if (parent->wall_ns > 0 &&
      (child->wall_ns == 0 || child->wall_ns > parent->wall_ns))
    return false;
  ...
  /* Token budget */
  if (parent->tokens > 0 &&
      (child->tokens == 0 || child->tokens > parent->tokens))
    return false;

  /* Tool calls budget */
  if (parent->tool_calls > 0 &&
      (child->tool_calls == 0 || child->tool_calls > parent->tool_calls))
    return false;
 ...
 */
static boolean ak_policy_is_subsumed(ak_policy_v2_t *parent,
                                     ak_policy_v2_t *child) {
  if (!parent || !child)
    return false;
...
static ak_delegated_cap_t *alloc_delegated_cap(void) {
  ak_delegated_cap_t *dcap =
      ak_alloc_zero(ak_process_state.h, ak_delegated_cap_t);
...
static void generate_run_id(u8 *run_id) { ak_generate_token_id(run_id); }
...
ak_process_entry_t *ak_process_register(u64 pid, u64 parent_pid,
                                        ak_policy_v2_t *policy, u32 flags) {
  if (!ak_process_state.initialized)
    return NULL;
  ...
  if (parent_pid > 0) {
    ...
    if (parent) {
      runtime_memcpy(entry->parent_run_id, parent->run_id, AK_TOKEN_ID_SIZE);
    }
  }
  ...
  ak_debug("ak_process: registered pid=%lu parent=%lu flags=0x%x", pid,
           parent_pid, flags);
...
void ak_process_unregister(u64 pid, int exit_code, int exit_signal) {
  ...
  while (entry) {
    if (entry->pid == pid) {
      ...
      ak_debug("ak_process: unregistered pid=%lu exit=%d sig=%d", pid,
...
int ak_process_set_policy(u64 pid, ak_policy_v2_t *policy) {
  ...
  if (entry->policy && !ak_policy_is_subsumed(entry->policy, policy)) {
    process_unlock();
    ak_warn(
        "ak_process: policy change denied - not a subset of current policy");
    return -EPERM;
  }
...
int ak_process_delegate_cap(u64 parent_pid, u64 child_pid, ak_capability_t *cap,
                            u64 ttl_ms) {
  if (!cap)
    return -EINVAL;
  ...
  ak_capability_t *child_cap = ak_capability_delegate(
      ak_process_state.h, cap, cap->type, (const char *)cap->resource,
      NULL, /* Same methods */
      effective_ttl, cap->rate_limit, cap->rate_window_ms);
...
boolean ak_process_authorize_spawn(u64 parent_pid,
  ...
  if (!parent) {
    ...
    runtime_strncpy(decision->missing_cap, "process.parent", AK_MAX_CAPSTR);
  ...
  if (!ak_process_has_cap(parent_pid, AK_CAP_SPAWN, req->program)) {
    ...
    if (!policy || !ak_policy_v2_check_spawn(policy, req->program)) {
      ...
      runtime_strncpy(decision->missing_cap, "spawn", AK_MAX_CAPSTR);
      ...
      runtime_strncpy(decision->detail, "Missing spawn capability for program",
                      AK_MAX_DETAIL);
      return false;
    }
  }
  ...
  if (req->child_policy && parent->policy) {
    if (!ak_policy_is_subsumed(parent->policy, req->child_policy)) {
      decision->reason_code = AK_DENY_NO_CAP;
      decision->errno_equiv = EPERM;
      runtime_strncpy(decision->missing_cap, "spawn.policy", AK_MAX_CAPSTR);
  ...
  if (ak_process_count_children(parent_pid) >= AK_PROCESS_MAX_CHILDREN) {
    ...
    runtime_strncpy(decision->missing_cap, "spawn.limit", AK_MAX_CAPSTR);
    runtime_strncpy(decision->detail, "Maximum child process limit reached",
                    AK_MAX_DETAIL);
    return false;
  }
...
int ak_process_spawn(u64 parent_pid, const ak_spawn_request_t *req,
...
u32 ak_process_count_children(u64 pid) {
  ...
  for (u32 i = 0; i < AK_PROCESS_MAX_ENTRIES; i++) {
    ak_process_entry_t *entry = ak_process_state.entries[i];
    while (entry) {
      if (entry->parent_pid == pid && entry->state == AK_PROC_STATE_RUNNING) {
        count++;
      }
      entry = entry->next;
    }
  ...
  for (u32 i = 0; i < AK_PROCESS_MAX_ENTRIES; i++) {
    ...
    while (entry) {
      rprintf("PID %lu (parent=%lu, state=%d, caps=%u)\n", entry->pid,
              entry->parent_pid, entry->state, entry->cap_count);
      rprintf("  run_id: %02x%02x%02x%02x...\n", entry->run_id[0],
              entry->run_id[1], entry->run_id[2], entry->run_id[3]);
      if (entry->program[0]) {
        rprintf("  program: %s\n", entry->program);
      }
