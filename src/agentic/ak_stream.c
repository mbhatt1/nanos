 ...
 */
static u64 get_time_ns(void) { return now(CLOCK_ID_MONOTONIC); }

/*
 * Generate unique session ID.
 */
static u64 generate_session_id(void) { return ++stream_state.session_counter; }
 ...
 */
static boolean check_rate_limit(ak_stream_session_t *session,
                                u64 additional_bytes, u64 additional_tokens) {
  ...
  if (session->bytes_per_second > 0) {
    if (session->rate_bytes_in_window + additional_bytes >
  ...
  if (session->tokens_per_second > 0) {
    if (session->rate_tokens_in_window + additional_tokens >
 ...
 */
static void update_rate_counters(ak_stream_session_t *session, u64 bytes,
                                 u64 tokens) {
 ...
 */
static boolean check_stop_sequences(ak_stream_session_t *session,
  ...
  if (session->stop_sequences) {
    for (u32 i = 0; i < session->stop_sequence_count; i++) {
      if (session->stop_sequences[i].sequence) {
        deallocate(session->session_heap, session->stop_sequences[i].sequence,
                   session->stop_sequences[i].sequence_len + 1);
      }
    }
    deallocate(session->session_heap, session->stop_sequences,
...
void ak_stream_init(heap h) {
  ...
  stream_state.default_budget.idle_timeout_ms =
      AK_STREAM_DEFAULT_IDLE_TIMEOUT_MS;
...
ak_stream_session_t *ak_stream_create(heap h, ak_stream_type_t type,
...
s64 ak_stream_close(ak_stream_session_t *session) {
  ...
  if (session->on_close_callback) {
    session->on_close_callback(session, AK_STREAM_STATE_CLOSED,
                               session->on_close_ctx);
  }

  return 0;
}

s64 ak_stream_abort(ak_stream_session_t *session, s64 error_code,
  ...
  if (error_msg) {
    runtime_strncpy(session->error_msg, error_msg,
                    sizeof(session->error_msg) - 1);
  ...
  /* Invoke close callback if registered */
  if (session->on_close_callback) {
    session->on_close_callback(session, AK_STREAM_STATE_ERROR,
                               session->on_close_ctx);
  }
...
s64 ak_stream_send_chunk(ak_stream_session_t *session, const u8 *data,
                         u64 len) {
  if (!session) {
    return AK_E_STREAM_INVALID;
  }
...
s64 ak_stream_send_typed_chunk(ak_stream_session_t *session,
                               ak_stream_chunk_t *chunk) {
  if (!session || !chunk) {
    return AK_E_STREAM_INVALID;
  }
  ...
  return ak_stream_send_chunk(
      session, chunk->data ? buffer_ref(chunk->data, 0) : NULL, len);
}

s64 ak_stream_send_sse_event(ak_stream_session_t *session,
                             const char *event_type, const char *event_id,
  ...
  if (event_type) {
    total_size +=
        7 + runtime_strlen(event_type) + 1; /* "event: " + type + "\n" */
  }
  if (event_id) {
...
s64 ak_stream_send_ws_frame(ak_stream_session_t *session, u8 opcode,
                            boolean fin, const u8 *data, u64 data_len) {
  if (!session) {
    return AK_E_STREAM_INVALID;
  }
...
s64 ak_stream_send_token(ak_stream_session_t *session, const char *token,
                         u32 token_len) {
  if (!session) {
    return AK_E_STREAM_INVALID;
  }
  ...
  if (check_stop_sequences(session, token, token_len)) {
    ...
    if (session->on_token_callback) {
      session->on_token_callback(session, token, token_len, true,
                                 session->on_token_ctx);
    }
  ...
  if (session->partial_token_buffer && session->partial_token_threshold > 0) {
    ...
    if (buffer_length(session->partial_token_buffer) >=
        session->partial_token_threshold) {
    ...
    if (session->on_token_callback) {
      session->on_token_callback(session, token, token_len, false,
                                 session->on_token_ctx);
    }
  }
...
s64 ak_stream_send_tokens(ak_stream_session_t *session, const char **tokens,
                          const u32 *token_lens, u32 count) {
...
s64 ak_stream_flush_tokens(ak_stream_session_t *session) {
  ...
  if (session->on_token_callback) {
    const char *flushed_data =
        (const char *)buffer_ref(session->partial_token_buffer, 0);
    session->on_token_callback(session, flushed_data, (u32)buffer_len, false,
                               session->on_token_ctx);
  }
...
s64 ak_stream_set_stop_sequences(ak_stream_session_t *session,
                                 const char **sequences, u32 count) {
  if (!session) {
    return AK_E_STREAM_INVALID;
  }
  ...
  session->stop_sequences =
      ak_alloc_array_zero(session->session_heap, ak_stop_sequence_t, count);
  ...
  for (u32 i = 0; i < count; i++) {
    if (sequences[i]) {
      ...
      session->stop_sequences[i].sequence =
          allocate(session->session_heap, len + 1);
...
boolean ak_stream_stop_triggered(ak_stream_session_t *session,
                                 u32 *triggered_index) {
  if (!session) {
    return false;
  }
...
void ak_stream_set_token_buffer_threshold(ak_stream_session_t *session,
                                          u32 threshold_bytes) {
  if (!session) {
    return;
  }
...
boolean ak_stream_budget_check(ak_stream_session_t *session,
                               u64 additional_bytes, u64 additional_tokens) {
  if (!session) {
    return false;
  }
...
void ak_stream_budget_remaining(ak_stream_session_t *session,
                                u64 *bytes_remaining, u64 *tokens_remaining) {
  if (!session) {
    if (bytes_remaining)
      *bytes_remaining = 0;
    if (tokens_remaining)
  ...
  if (bytes_remaining) {
    if (session->bytes_limit > 0 &&
        session->bytes_sent < session->bytes_limit) {
...
s64 ak_stream_budget_update(ak_stream_session_t *session,
...
boolean ak_stream_rate_check(ak_stream_session_t *session, u64 bytes,
                             u64 tokens) {
...
void ak_stream_get_stats(ak_stream_session_t *session,
  ...
  if (session->chunks_sent > 1 && stats->duration_ns > 0) {
    stats->avg_chunk_interval_ns =
        stats->duration_ns / (session->chunks_sent - 1);
  }
  ...
  if (rate_window_elapsed > 0) {
    stats->current_bytes_per_sec =
        (session->rate_bytes_in_window * BILLION) / rate_window_elapsed;
    stats->current_tokens_per_sec =
        (session->rate_tokens_in_window * BILLION) / rate_window_elapsed;
  }
...
u32 ak_stream_list_active(ak_stream_session_t **sessions, u32 max_count) {
...
void ak_stream_on_chunk(ak_stream_session_t *session,
                        ak_stream_on_chunk_t callback, void *ctx) {
  if (!session)
    return;
...
void ak_stream_on_close(ak_stream_session_t *session,
                        ak_stream_on_close_t callback, void *ctx) {
  if (!session)
    return;
...
void ak_stream_on_token(ak_stream_session_t *session,
                        ak_stream_on_token_t callback, void *ctx) {
  if (!session)
    return;
...
s64 ak_stream_cancel(ak_stream_session_t *session, const char *reason) {
  ...
  if (reason) {
    runtime_strncpy(session->cancel_reason, reason,
                    sizeof(session->cancel_reason) - 1);
...
boolean ak_stream_can_send(ak_stream_session_t *session, u64 bytes_pending) {
  ...
  if (session->backpressure_active &&
      bytes_pending < session->backpressure_high_water / 2) {
...
void ak_stream_set_backpressure_limit(ak_stream_session_t *session,
                                      u64 high_water) {
  if (!session) {
    return;
  }
