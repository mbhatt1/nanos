...
buffer ak_compress_lz4(heap h, buffer input) {
  ...
  u8 size_hdr[4];
  write32(size_hdr,
          (u32)src_len); /* Safe: checked src_len <= UINT32_MAX above */
  ...
  while (ip < mflimit) {
    ...
    if (ref_idx > 0 && ip - ref < 65535 &&
      ...
      u64 match_len =
          LZ4_MINMATCH +
          count_match(ip + LZ4_MINMATCH, ref + LZ4_MINMATCH, match_limit);
...
buffer ak_decompress_lz4(heap h, buffer compressed) {
  ...
  if (compressed_len > 0 &&
      (u64)original_size / compressed_len > AK_COMPRESS_MAX_RATIO)
    return 0;
...
buffer ak_decompress_lz4_sized(heap h, buffer compressed, u64 original_size) {
  ...
  if (compressed_len > 0 &&
      original_size / compressed_len > AK_COMPRESS_MAX_RATIO)
    return 0;
