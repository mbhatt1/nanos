...
static inline void spin_lock_init(u64 *lock) { *lock = 0; }
...
static inline void spin_lock_release(u64 *lock) { __sync_lock_release(lock); }
...
static boolean name_matches_pattern(const char *name, const char *pattern) {
  ...
  if (pattern[0] == '*') {
    ...
    if (name_len < suffix_len)
      return false;
    return runtime_strncmp(name + name_len - suffix_len, suffix, suffix_len) ==
           0;
  }
...
int ak_agent_register(ak_agent_context_t *ctx, const char *name,
  ...
  if (discoverable) {
    for (u32 i = 0; i < REGISTRY_HASH_SIZE; i++) {
      ...
      while (e) {
        if (e->active && e->discoverable && ak_strcmp(e->name, name) == 0) {
          spin_lock_release(&ipc_state.registry_lock);
  ...
  ak_agent_registry_entry_t *entry =
      allocate(ipc_state.h, sizeof(ak_agent_registry_entry_t));
 ...
 */
static s64 ipc_send_internal(ak_agent_context_t *ctx, u64 receiver_pid,
                             u32 msg_type, u32 flags, u64 correlation_id,
                             buffer payload) {
  if (!ipc_state.initialized)
    return AK_E_IPC_NOT_INIT;
  ...
  if (!is_parent && !is_child) {
    ...
    if (ak_ctx && ak_ctx->mode != AK_MODE_OFF) {
      ...
      long retval;
      int auth_result =
          ak_authorize_and_execute(ak_ctx, &req, &decision, &retval);
  ...
  ak_debug("ak_agent_ipc: sent msg_id=%lx from=%lx to=%lx type=%u", msg->msg_id,
           sender_pid, receiver_pid, msg_type);

  return (s64)msg->msg_id;
}

s64 ak_ipc_send(ak_agent_context_t *ctx, u64 receiver_pid, u32 msg_type,
                u32 flags, buffer payload) {
  return ipc_send_internal(ctx, receiver_pid, msg_type, flags, 0, payload);
}

int ak_ipc_send_request(ak_agent_context_t *ctx, u64 receiver_pid, u32 msg_type,
                        buffer payload, u64 *correlation_out) {
...
int ak_ipc_send_response(ak_agent_context_t *ctx, u64 receiver_pid,
                         u64 correlation_id, buffer payload) {
  s64 result = ipc_send_internal(ctx, receiver_pid, AK_IPC_MSG_TYPE_RESPONSE, 0,
                                 correlation_id, payload);
  return result < 0 ? (int)result : 0;
}

s64 ak_ipc_broadcast(ak_agent_context_t *ctx, u32 msg_type, u32 flags,
                     buffer payload) {
  if (!ipc_state.initialized)
    return AK_E_IPC_NOT_INIT;
  ...
  for (u32 i = 0;
       i < REGISTRY_HASH_SIZE && recipient_count < AK_IPC_BROADCAST_LIMIT;
       i++) {
    ak_agent_registry_entry_t *entry = ipc_state.registry_by_pid[i];
...
ak_ipc_message_t *ak_ipc_recv(ak_agent_context_t *ctx, s32 timeout_ms) {
  if (!ipc_state.initialized)
    return NULL;
...
ak_ipc_message_t *ak_ipc_recv_correlated(ak_agent_context_t *ctx,
                                         u64 correlation_id, s32 timeout_ms) {
  if (!ipc_state.initialized)
    return NULL;
...
s64 ak_agent_discover(const char *pattern, u64 *pids_out, u32 max_pids) {
  if (!ipc_state.initialized)
    return AK_E_IPC_NOT_INIT;
...
int ak_agent_get_name(u64 pid, char *name_out, u32 max_len) {
  if (!ipc_state.initialized)
    return AK_E_IPC_NOT_INIT;
...
s64 ak_agent_get_children(ak_agent_context_t *ctx, u64 *pids_out,
                          u32 max_pids) {
  if (!ipc_state.initialized)
    return AK_E_IPC_NOT_INIT;
...
int ak_agent_add_child(ak_agent_context_t *parent_ctx, u64 child_pid) {
  if (!ipc_state.initialized)
    return AK_E_IPC_NOT_INIT;
...
int ak_agent_remove_child(ak_agent_context_t *parent_ctx, u64 child_pid) {
  if (!ipc_state.initialized)
    return AK_E_IPC_NOT_INIT;
...
int ak_ipc_set_inbox_limit(ak_agent_context_t *ctx, u32 limit) {
  if (!ipc_state.initialized)
    return AK_E_IPC_NOT_INIT;
...
int ak_ipc_get_mailbox_stats(ak_agent_context_t *ctx, u32 *count_out,
                             u32 *limit_out, u64 *received_out,
                             u64 *dropped_out) {
  if (!ipc_state.initialized)
    return AK_E_IPC_NOT_INIT;
  ...
  if (count_out)
    *count_out = mb->inbox_count;
  if (limit_out)
    *limit_out = mb->inbox_limit;
  if (received_out)
    *received_out = mb->messages_received;
  if (dropped_out)
    *dropped_out = mb->messages_dropped;
...
int ak_ipc_grant_capability(ak_agent_context_t *ctx, u64 *peer_pids,
                            u32 peer_count, boolean can_broadcast) {
  if (!ipc_state.initialized)
    return AK_E_IPC_NOT_INIT;
  ...
  if (peer_pids && peer_count > 0) {
    ...
    if (!entry->allowed_peer_pids ||
        ak_is_invalid_address(entry->allowed_peer_pids)) {
      entry->allowed_peer_pids = NULL;
      spin_lock_release(&ipc_state.registry_lock);
      return -ENOMEM;
    }
    ...
    /* Copy peer PIDs */
    runtime_memcpy(entry->allowed_peer_pids, peer_pids,
                   peer_count * sizeof(u64));
    entry->allowed_peer_count = peer_count;
    entry->allowed_peer_capacity = capacity;

    ak_debug(
        "ak_ipc_grant_capability: pid=%lx granted access to %u specific peers",
        pid, peer_count);
  } else {
    /*
    ...
    ak_debug(
        "ak_ipc_grant_capability: pid=%lx granted unrestricted peer access",
        pid);
  }
...
int ak_effect_from_agent_send(ak_effect_req_t *req, ak_ctx_t *ctx,
                              u64 receiver_pid, u32 msg_type) {
  if (!req)
    return -EINVAL;
