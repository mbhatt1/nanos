  ...
  for (u64 i = 0; i < len; i++) {
    if (str[i] == '\\' && i + 1 < len) {
      ...
      case '/':
        ...
        break;
      case 'b':
        buffer_write(out, "\b", 1);
        written++;
        i++;
        break;
      case 'f':
        buffer_write(out, "\f", 1);
        written++;
        i++;
        break;
      case 'u':
...
static s64 json_get_string(buffer json, const char *key, char *out,
                           u64 out_len) {
...
s64 ak_proxy_http_get(const char *url, buffer headers_json,
...
s64 ak_proxy_http_request(const char *method, const char *url,
  ...
  err =
      virtio_read_line(ak_proxy_state.virtio_handle, resp, AK_PROXY_TIMEOUT_MS);
  if (err < 0) {
...
s64 ak_proxy_fs_write(const char *path, buffer content) {
  ...
  json_escape_string(req, (char *)buffer_ref(content, 0),
...
s64 ak_proxy_fs_list(const char *path, ak_file_info_t **entries, u64 *count) {
  ...
  *count = 0;
  ...
  u64 pos = 0;
  while (pos < arr_len && (arr_data[pos] == '[' || arr_data[pos] == ' ' ||
                           arr_data[pos] == '\t' || arr_data[pos] == '\n'))
    pos++;
  ...
  ak_file_info_t *result =
      allocate(ak_proxy_state.h, entry_count * sizeof(ak_file_info_t));
    ...
    if (arr_data[i] == '"') {
      ...
      if (depth == 0) {
        ...
        if (obj) {
          ...
          json_get_string(obj, "name", result[entry_idx].name,
                          sizeof(result[entry_idx].name));

          s64 size = 0;
          json_get_number(obj, "size", &size);
          result[entry_idx].size = size;

          json_get_string(obj, "mode", result[entry_idx].mode,
                          sizeof(result[entry_idx].mode));
...
s64 ak_proxy_llm_complete(const char *model, const char *prompt, u64 max_tokens,
  ...
  if (data) {
    ...
    json_get_string(data, "finish_reason", response->finish_reason,
...
s64 ak_proxy_llm_chat(const char *model, buffer messages_json, u64 max_tokens,
  ...
  /* Build request */
  buffer req =
      allocate_buffer(ak_proxy_state.h, buffer_length(messages_json) + 512);
  if (!req)
    return -ENOMEM;
...
s64 ak_proxy_llm_stream(const char *model, const char *prompt, u64 max_tokens,
  ...
  while (!done) {
    ...
    err = virtio_read_line(ak_proxy_state.virtio_handle, chunk_buf,
                           AK_PROXY_TIMEOUT_MS);
    if (err < 0) {
...
s64 ak_proxy_tcp_connect(const char *host, u16 port,
  ...
  if (data) {
    ...
    json_get_string(data, "local_addr", conn->local_addr,
                    sizeof(conn->local_addr));
    json_get_string(data, "remote_addr", conn->remote_addr,
...
s64 ak_proxy_tcp_send(u64 conn_id, const void *data, u64 len) {
  ...
  /* Build request */
  buffer req =
      allocate_buffer(ak_proxy_state.h, len * 2 + 256); /* Base64 overhead */
  if (!req)
    return -ENOMEM;
